    ...
    while (! feof(arqAcessos))
    {
      ...
      if ((result != 0) && (result != EOF))
      {
        if (tipoAcesso == 'I'){	// Acesso é leitura de uma instrução
          ...
          resultAcessoI = buscaInsereCache(&cacheI1, endereco);
          if ((resultAcessoI == 1) || (resultAcessoI == 2))	// Se houve falha na cache
            nFalhasI1 ++;
        }
          ...
          // Procura bloco na cache e, se não encontra, insere bloco na cache
          // Valor de retorno = 0 (acerto), 1 (falha sem substituição) ou 2 (falha com substituição)
          resultAcessoD = buscaInsereCache(&cacheD1, endereco);
          if ((resultAcessoD == 1) || (resultAcessoD == 2))	// Se houve falha na cache
            nFalhasD1 ++;
        }
      }
    }
  ...
  void inicializa(int argc, char **argv)
  {
    ...
    // Lê arquivo de configuracao e obtem nBlocos, associatividade e nPalavrasBloco da cache I1
    ...
    // Aloca e inicializa estrutura de dados da cache I1
